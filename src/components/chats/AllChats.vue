<template>
  <div class="scroll relative mx-auto h-screen w-full bg-brand-dark">
    <div
      class="mb-2 flex h-fit w-full items-center justify-between rounded-lg bg-brand p-4">
      <h2 class="my-10 text-6xl font-black">Chats History</h2>
      <RouterLink class="cursor-pointer" :to="{ name: 'user' }">
        <img
          :src="state.user?.profile_picture"
          class="block h-[4rem] w-[4rem] rounded-full bg-white"
          alt=""
          v-if="state.user?.profile_picture" />
        <p class="material-icons" style="font-size: 50px" v-else>
          account_circle
        </p>
      </RouterLink>
    </div>
    ...
    <div
      ...
      <img :src="LogoIcon" class="mx-auto block h-[16rem] w-[16rem]" alt="" />
